...
int main(int argc, char *argv[])
{
    ...
    /* 1.1. Read arguments */
    if (argc<4) {
        fprintf(stderr,"Usage: %s <num_threads> <size> <storm_1_file> [ <storm_i_file> ] ... \n", argv[0] );
        exit( EXIT_FAILURE );
    }
    ...
    //pode nao valer a pena paralelizar
    ...
    for( i=0; i<num_storms; i++) {
      ...
      // (DEsc 20210522: TEM DEPENDENCIAS com todas as mudancas a layer e layer_copy)
      // (DEsc 20210522: TEM DEPENDENCIAS em todos os loops menos 4.3)

        /* 4.1. Add impacts energies to layer cells */
        /* For each particle */
        /* DR 20210517: Loop Dependency (Output Dep.)
         * - The same memory location layer(k) is written in update and then written again in the next iteration of the loop
         */

        // (DR 20210521: RETIREI ESTE PARALLEL CYCLE POIS NAO ME PARECEU TRAZER MELHORIAS e tem o OutputDep. no layer[k])
        // (DEsc 20210522: NAO TEM OutputDep. no layer[k] pq se trata de uma soma acumulada e a ordem das parcelas pode ser trocada)
        // PRECISO VERIFICAR SE COMPENSA PARALELIZAR
        //#pragma omp parallel for
        for( j=0; j<storms[i].size; j++ )
        {
            ...
            /* DR 20210517: No Loop Dependency
             * - No same memory location is written or read at each loop iteration
             * - It reads and updates each position individually
             */
            #pragma omp parallel for
            for( k=0; k<layer_size; k++ )
            {
        ...
        for (k = 1; k < layer_size - 1; k++)
        {
            ...
            {
                if (layer[k] > max[omp_get_thread_num()])
                {
                    max[omp_get_thread_num()] = layer[k];
                    pos[omp_get_thread_num()] = k;
                }
            }
        }
